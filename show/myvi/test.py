...
def dem():
	...
	vts, fs, ns, cs = show.util.build_surf2d(img, ds=1, k=0.3, sigma=2)
...
def volume():
	...
	vts, fs, ns, vs = show.util.build_surf3d(imgs, 1, 80)
...
def ball():
	vts, fs, ns, cs = show.build_ball((100, 100, 100), 50, (1, 0, 0))
	manager = show.Manager()
...
def random_balls():
	...
	cs = show.linear_color('jet')[cs] / 255

	vts, fs, ns, cs = show.build_balls(os, rs, cs)
	manager = show.Manager()
	manager.add_surf('balls', vts, fs, ns, cs)
...
def line():
	...
	vts, fs, ns, cs = show.build_line(x, y, z, (1, 0, 0))
	cs[:] = show.auto_lookup(vts[:, 2], show.linear_color('jet')) / 255
...
def mesh():
	...
	vts, fs, ns, cs = show.build_mesh(x, y, z)
	cs[:] = show.util.auto_lookup(vts[:, 2], show.util.linear_color('jet')) / 255
...
def ball_ring_box():
	...
	vts_b, fs_b, ns_b, cs_b = show.build_balls(list(os), list(rs), list(cs))
	vts_l, fs_l, ns_l, cs_l = show.build_line(os[:, 0], os[:, 1], os[:, 2], list(cs))
	vts_c, fs_c, ns_c, cs_c = show.build_cube((0, 0, 0), (1, 1, 1))
	manager = show.Manager()
...
def balls_with_mark():
	...
	vts_b, fs_b, ns_b, cs_b = show.build_balls(os, rs, cs)
	cont = ['ID:%s'%i for i in range(10)]
	vtss, fss, pps, h, color = show.build_marks(cont, os, rs, 0.05, (1, 1, 1))
	manager = show.Manager()
	manager.add_surf('balls', vts_b, fs_b, ns_b, cs_b)
...
def surface2d():
	...
	z = x * np.exp( - x**2 - y**2)
	vts, fs, ns, cs = show.util.build_surf2d(z, ds=1, k=20, sigma=2)
	cs[:] = show.util.auto_lookup(vts[:, 2], show.util.linear_color('jet')) / 255
	manager = show.Manager()
	manager.add_surf('dem', vts, fs, ns, cs)
	manager.show('DEM Demo') 
...
def arrow():
	...
	vts, fs, ns, c = show.util.build_arrows(v1, v2, 1, 1, 1, 1, (1, 0, 0))
	manager = show.Manager()
	manager.add_surf('dem', vts, fs, ns, c)
	manager.show('DEM Demo') 

def cube():
	vts, fs, ns, cs = show.build_cube((0, 0, 0), (1, 1, 1))
	manager = show.Manager()
...
def cube_surf():
	...
	vts, fs, ns, vs = show.util.build_surf3d(imgs, 1, 128)
	manager = show.Manager()
	manager.add_surf('vessel', vts, fs, ns, (1,0,0))
	'''
	manager = show.Manager()
	vts, fs, ns, cs = show.build_img_cube(imgs)
	obj = manager.add_surf('cube', vts, fs, ns, cs)
	vts, fs, ns, cs = show.build_img_box(imgs)
